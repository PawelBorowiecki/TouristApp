package com.pawel.touristapp.weather.utils
...
import com.pawel.touristapp.weather.Constants
import com.pawel.touristapp.weather.model.WeatherData
import com.pawel.touristapp.weather.model.WeatherViewModel

@Composable
fun WeatherScreen(location: String, activity: CheckingWeatherActivity, viewModel: WeatherViewModel = viewModel()){
...
private fun WeatherItem(weather: WeatherData, onDelete: () -> Unit){
    OutlinedCard(
        ...
        modifier = Modifier.fillMaxWidth().height(250.dp).padding(8.dp, 4.dp, 8.dp, 4.dp).background(brush = Brush.radialGradient(colors = listOf(
            Color.Transparent, Color.White), tileMode = TileMode.Clamp))
    ) {
...
@Composable
fun NotificationToast(context: Context, location: String, activity: CheckingWeatherActivity){
    var permissionGranted by remember { mutableStateOf(activity.checkNotificationPermission()) }
    val requestPermissionLauncher = rememberLauncherForActivityResult(ActivityResultContracts.RequestPermission()) { isGranted ->
        permissionGranted = isGranted
    }

    if(permissionGranted){
        activity.sendNotification(context, location)
    }else{
